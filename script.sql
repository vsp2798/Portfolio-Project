...
ORDER BY 1,
	2 DESC -- This query says that at the end of October that If you contract covid and live in Canada there is a 1.68 % chance of dying.
 -------------------------------------------------------------------------
...
WHERE NEW_CASES IS NOT NULL
	AND CONTINENT IS NOT NULL --Group By date1
ORDER BY 1,2 -------------------------------------------------------------------------
...
SELECT CD.CONTINENT,
	...
	CD.POPULATION,
	VC.NEW_VACCINATIONS,
	SUM(VC.NEW_VACCINATIONS) OVER (PARTITION BY CD.LOCATION1
																																ORDER BY CD.LOCATION1,
																																	CD.DATE1) AS ROLLINGPEOPLEVACCINATED
FROM COVIDDEATHS CD
...
ORDER BY 2,3 -------------------------------------------------------------------------
 ...
 -- CTE is used. Also this query finds out that only 70% of population has been vaccinated as of Nov 1st 2021
 WITH POPULATION_VS_VACCINATIONS (CONTINENT,
